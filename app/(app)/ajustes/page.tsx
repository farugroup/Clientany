        ...
        {done ? (
          ...
          <>
            ...
            <div className="mt-4 rounded-xl bg-ink-850 p-3">
              <div className="label mb-2">Cómo conectarlo</div>
              <ol className="space-y-1.5">
                {def.steps.map((s, i) => (
                  <li key={i} className="flex gap-2 text-xs text-ink-300">
                    <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-brand-500/20 text-[10px] font-bold text-brand-300">
                      {i + 1}
                    </span>
                    {s}
                  </li>
